...
const handleStart = async () => {
  try {
    ...
    await initDb(); // only call this
    console.log("✅ DB ready. Navigating...");
    router.replace("/home");
  } catch (error) {
  ...
  return (
    <SafeAreaView className="bg-[#F9F9F9] h-full">
      ...
      >
        <View className="">
          <View className="px-6 absolute z-10 top-10 right-5 items-center flex-row">
            <Image
              source={icons.logo}
              className="w-6 h-6"
              resizeMode="contain"
          ...
          <View className="-mt-[100px] justify-center items-center px-6">
            <Text className="text-[40px] leading-[150%] text-center font-bribold max-w-[335px] text-black-300">
            ...
            <Text className="max-w-[342px] text-center text-black-100 text-[16px] leading-[24px] mt-5 font-briregular">
              Capture product images, update stock, and {"\n"}stay organized
              effortlessly.
            </Text>
            <View className="mt-10 w-full justify-center items-center gap-5">
              <CustomButton
